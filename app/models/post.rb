class Post < ActiveRecord::Base
  ...
  def unique_impression_count(start_date=nil,end_date=nil)
    start_date.blank? ? impressions.group(:ip_address, :impressionable_id, :id, :impressionable_type, :user_id, :created_at, :updated_at).all.size : impressions.where("created_at>=? and created_at<=?",start_date,end_date).group(:ip_address, :impressionable_id, :id, :impressionable_type, :user_id, :created_at, :updated_at).all.size
  end
