class ApplicationController < ActionController::Base
  ...
  def top_lists
    @top5_posts_of_week = Post.order("created_at ASC").find_all_by_id(Impression.top5("Post", (Time.now - 7.day), Time.now))
    @top5_openings_of_week = Opening.order("created_at ASC").find_all_by_id(Impression.top5("Opening", (Time.now - 7.day), Time.now))
